...
### Songs & charts
- **PWA + mobile play**: the game is installable (web manifest, icons, and a service worker that caches hashed assets so it opens offline). On phones/tablets the playfield becomes a **tap surface** — the field splits into one touch column per lane with multi-touch chords and holds — and only five-key charts are offered (keyboard/letters modes need a physical keyboard). The chart editor is desktop-only, and tap input never activates on desktop.
- **Menu music & UI sounds**: a chill background loop plays on every menu screen (silenced during gameplay, the editor, and song previews), menu items click and hover with sound effects, and both are adjustable under Settings → Audio (music volume slider, UI-sounds toggle). Files in `src/audio/` **without** a `Title - Artist` separator are treated as system sounds, not library songs.
...
- **Song select browsing**: a two-column layout — leaderboards on the left (**difficulty filter chips** above the global board, local scores collapsed beneath), songs on the right in a contrasting amber-tinted column (filter box, **genre dropdown**, scroll-wheel/arrow-key selection, cards showing genre/duration/BPM with a **play button** per card). The **album art doubles as a preview button** — click it to hear a faded 12-second snippet from the hook of the song. Pressing play opens a **setup dialog** to pick difficulty, modifiers, and practice options before the run starts. The whole app sits on a subtle diagonal cross-gradient backdrop with five wavy lane-colored lines and scattered note-dots, and even the scrollbar is an amber gem-pill.
...
- **Chart editor**: a **song picker dropdown** to switch between any song in the library without leaving the editor (with an unsaved-changes guard), vertical timeline with **waveform**, a **transport scrubber** (play/pause + draggable seek bar with live timecode) to jump anywhere in the song while placing notes, BPM editor + tap-BPM, offset adjustment, snap divisions **1/1 – 1/32 incl. 1/24**, click-to-place, drag-to-move, drag-down for holds, right-click delete, box select, copy/paste, undo/redo, multiple difficulty charts per song (Easy/Medium/Hard/Expert × mode), instant **test play (F5)** that returns to the editor, JSON export.
- **⏺ Live recording**: press Record in the editor and the song plays while you tap your actual lanes — every press lands as a note (latency-compensated and snapped to the current snap division), long presses become hold notes, and chords are just simultaneous presses. Esc (or Record again, or the song ending) stops the take; the whole take is one Ctrl+Z undo step.
- **Admin publishing**: when the admin is logged in (Settings → Admin), the editor's Save button becomes **📤 Save & Publish** — saving uploads the song's charts to the server as the **canonical version every player plays and is ranked on**. Other clients pull published charts in the background from Song Select and replace their local copies; the song's BPM/offset are adopted from the published version too, so the notes compile to identical times on every machine (important because bundled songs auto-detect tempo per client, which would otherwise drift). API: `GET /api/charts?songId=…` (public), `POST`/`DELETE /api/charts` (admin).
...
- Reported device output latency (`AudioContext.outputLatency`) is compensated automatically — sampled and locked at the instant playback begins (when the device's estimate is reliable), so the very first note is judged exactly when it reaches the line. A manual **audio offset** and separate **visual offset** are in Settings for fine calibration. The judged/heard song time is `rawMs − outputLatency − audioOffset`, so a key struck when you hear a beat lands at delta ≈ 0; with default offsets the visual line-crossing coincides with judge-perfect. (This relationship is covered by a headless Conductor test.)
